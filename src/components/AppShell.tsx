...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (ready && !currentUserId) router.replace("/");
  ...
  return (
    <div className="min-h-dvh bg-n50">
      {/* ---------------------------------------------------------- Sidebar */}
      <aside className="fixed inset-y-0 start-0 z-40 hidden lg:flex w-[268px] flex-col bg-white border-e border-n200">
        <div className="px-5 py-4 border-b border-n100">
          <Link href="/dashboard" className="flex items-center gap-3">
            <Image
              src="/brand/dga-emblem.png"
              alt=""
              width={36}
              height={36}
      ...
      {open ? (
        <div className="lg:hidden fixed inset-0 z-50 no-print">
          ...
          <aside className="absolute inset-y-0 start-0 w-[280px] bg-white flex flex-col animate-in">
      ...
      <div className="lg:ms-[268px] flex flex-col min-h-dvh">
        <header className="app-header sticky top-0 z-30 bg-white/95 backdrop-blur border-b border-n200">
          <div className="flex items-center gap-3 px-4 sm:px-6 h-14">
            ...
            <span className="hidden sm:flex items-center gap-2.5 ps-3 border-s border-n200">
              <span className="w-8 h-8 rounded-full bg-dga-navy text-white grid place-items-center text-[12px] font-bold">
                {initials(user.name)}
              </span>
  ...
  return (
    <div className="border-t border-n100 p-3">
      <div className="flex items-center gap-3 rounded-[10px] bg-n50 p-3">
        <span className="w-9 h-9 shrink-0 rounded-full bg-dga-navy text-white grid place-items-center text-[12.5px] font-bold">
          {initials(user.name)}
        </span>
        ...
        <button
          ...
          className="shrink-0 p-2 rounded-[8px] text-n500 hover:bg-white hover:text-dga-red transition-colors"
        >
          <LogOut size={16} />
        </button>
      </div>
    ...
    <nav className="flex items-center gap-1.5 mb-4 text-[12.5px] text-n500 no-print flex-wrap">
      {items.map((it, i) => (
        <span key={i} className="flex items-center gap-1.5">
          ...
          {it.href ? (
            <Link href={it.href} className="hover:text-dga-navy font-semibold transition-colors">
              {it.label}
            </Link>
          ) : (
